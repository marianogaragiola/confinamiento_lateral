...
program cl1e
  ...
  integer, allocatable :: idx(:,:), idnm(:,:)
  integer :: n, m, N_lambda, ind_lambda
  real(pr), parameter :: alpha = 658.4092645439_pr, a0 = 0.0529177210_pr, eV = 27.21138564_pr
  real(pr) :: me, V0, omega, sigma, lambda_i, lambda_f, B_campo
  real(pr) :: b, l, delta_lambda, lambda
...
!!calculo los parametros dependientes
  ...
  allocate(idx((N_base+1)**2,3), idnm(N_base+1,N_base+1))
...
!!calculo el hamiltoniano
  ...
  open(11, file='./resultados/auval_vs_lambda-B'//B2//B1//'T.dat')
  write(11,*) '# me =', me
  write(11,*) '# V0 =', V0*eV
  ...
  do ind_lambda = 1, N_lambda
    ...
    call hamiltoniano(N_base, N_dim, lambda, idnm, H_e, V_ef, H);
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine indices(N_base, idx, idnm )
  implicit none
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine hamiltoniano(N_base, N_dim, lambda, idnm, H_e, V_ef, H)
  use precision
  implicit none
  integer, intent(in) :: N_base, N_dim
  integer, intent(in) :: idnm(N_base+1,N_base+1)
...
!      write(21,*) n1-1, m1-1, n2-1, m2-1, V_ef(ind1, ind2)
