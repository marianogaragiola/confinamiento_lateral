...
v0 = 0.001; ## Profundidad del pozo en eV
...
N_intervalos_r = 20; N_intervalos_z = 50;
N_cuad = 100;
grado = 4;
kord = grado + 1;
...
auval = np.array([[auval[i][j] for i in range(1,np.size(bcampo_vec)+1)] for j in range(N_dim)]);
...
np.savetxt('./resultados/auval-cl1e.dat', np.transpose(auval))
