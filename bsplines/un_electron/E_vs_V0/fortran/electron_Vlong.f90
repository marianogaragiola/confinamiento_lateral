! Codigo que calcula los autovalores de un electron en un pozo con forma gaussiana.
!
...
module carga
integer :: kord, lum, intg, nev, numero_puntos_V0
real(8) :: zmin, zmax, V0, sigma, V0i, V0f, z0, delta, c, gamma, me
integer :: l
character(1) :: tip
end module  carga
...
implicit none
integer :: i, j, ind
real(8), parameter :: a0 = 0.0529177210d0, eV = 27.21138564d0
real(8) :: time
...
numero_puntos_V0 = num_puntos_V0_
...
!###########################################################
!###########################################################
open(9,file = './resultados/electron-autovalores.dat')
!###########################################################
!###########################################################
...
write(9,*) '# profundidad inicial=', V0i*eV

write(9,*) '# profundidad final=', V0f*eV
...
delta = (V0f-V0i)/dble(numero_puntos_V0)

call  KNOTS_PESOS( kord, tip, gamma, zmin, zmax, c, l, lum, intg, t, k, x, w, pl)

do ind = 0, numero_puntos_V0

  V0 = V0i + dble(ind)*delta;
...
!###########################################################
!###########################################################
open(31,file = './resultados/electron-autovalores.dat', position = 'append')
!###########################################################
!###########################################################
!###########################################################
...
!!!!!!!!!!!!!!!!!!!!!!!####################################
!!!!!!!!!!!!!!!!!!!!!!!####################################
write(31,6) eV*V0, (eV*e(m),m = 1, nev)
!!!!!!!!!!!!!!!!!!!!!!!####################################
!!!!!!!!!!!!!!!!!!!!!!!####################################
!!!!!!!!!!!!!!!!!!!!!!!####################################
