...
real(8) :: zmin, zmax, V1, V2, sigma, eta, B_campo_i, B_campo_f
real(8) :: az, bz, r0, delta
real(8) :: me, B_campo, delta_B, c, gamma, l_campo
real(8) :: Tr, Ur, Vr, Hr, omega
integer :: l_interval
...
real(8), parameter :: c_light = 137.035999074492, ua_to_T = 1.72e3
real(8) :: time
...
V1 = V1_ ! profundidad del pozo en eV

V2 = V2_

az = az_; bz = bz_; r0 = r0_;
...
!###########################################################
!###########################################################
write(9,'(A28,f8.2,A1,f8.2,A4)') "# Intervalo de integracion:[", zmin,",", zmax, "] nm"
write(9,'(A32,x,I2,x,A33)') "# Tipo de distribucion de knots:", tipo, "(1 es uniforme, 2 es exponencial)"
write(9,'(A47,x,f8.6)') "# Cte de decaimiento en distribucion exp gamma =", gamma
write(9,'(A30,x,I2)') "# Orden de los bsplines kord =", kord
write(9,'(A19,x,I3,x,A24,x,I3)') "# Num de intervalos", l_interval, ", grado de la cuadratura", intg
write(9,'(A14,x,I3,x,A24,x,I3)') "# Num de knost", nk, ", tamaño de la base nb =", nb
write(9,'(A33,x,f8.6,x,A2)') "# Masa efectiva del electron me =", me, "UA"
write(9,'(A22,x,f6.4,x,A2)') "# Altura del pozo V1 =", v1, "eV"
write(9,'(A27,x,f6.4,x,A2)') "# Profundidad del pozo V2 =", v2, "eV"
write(9,'(A22,x,f6.4,x,A9,x,f6.4,x,A2)') "# Radios del pozo az =", az, "nm y bz =", bz, "nm"
write(9,'(A36,x,f6.4,x,A2)') "# Parametro de rectificacion delta =", delta, "nm"
write(9,'(A22,x,f6.4,x,A2)') "# Carga efectiva eta =", eta
write(9,'(A27,x,f6.2,x,A19,x,f7.2)') "# Campo inicial b_campo_i =", b_campo_i, "y final b_campo_f =", b_campo_f
write(9,'(A24)') "# Autovalores calculados"
call flush();
!###########################################################
!###########################################################
!###########################################################
      ...
      if( im>0.and.im<nb+1 ) then
        do n = 1,kord
          ...
          if( in>0.and.in<nb+1 )then
            ...
            if(abs(zz)<=0.5*az) then
              v01(im,in) = v01(im,in) - v2*w(k(i),j)*sp(m)*sp(n)
            elseif(0.5*az<abs(zz).and.abs(zz)<=0.5*(az+bz))then
              v01(im,in) = v01(im,in) + v2*w(k(i),j)*sp(m)*sp(n)
            end if
...
real(8), allocatable, dimension(:,:) :: s_copy, mh, ms, mv, hsim
real(8) :: ground_state
character(150) :: archivo1e, archivo2e
...
allocate( s_copy(dp,dp), hsim(NN,NN), ms(NN,NN), mv(NN,NN))
...
do m = 1, dp
  do n = 1, m
    mh(m,n) = ke(m,n) + v01(m,n) + Hr*s(m,n)
    mh(n,m) = mh(m,n)
  end do
end do
