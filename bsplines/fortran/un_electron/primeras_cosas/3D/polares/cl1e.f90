...
#ifndef L_INTERVAL_Z_
#define L_INTERVAL_Z_ 30
#endif

#ifndef L_INTERVAL_R_
#define L_INTERVAL_R_ 30
#endif
...
program main
  ...
  use mod_eig
  implicit none
  integer :: tipo, kord, l_interval_z, l_interval_r, n_cuad, nev
  integer :: num_puntos_B
  real(pr) :: zmin, zmax, rmin, rmax, beta, me, sigma, v0, b_campo_i, b_campo_f
  ...
  rmin = real(RMIN_, pr); rmax = real(RMAX_, pr);
  tipo = TIPO_;
  beta = real(BETA_, pr);
  kord = KORD_;
  l_interval_z = L_INTERVAL_Z_; l_interval_r = L_INTERVAL_R_;
  n_cuad = N_CUAD_; nev = NEV_;
  me = real(ME_, pr); sigma = real(SIGMA_, pr);
  v0 = real(V0_, pr);
  b_campo_i = real(B_CAMPO_I_, pr); b_campo_f = real(B_CAMPO_F_, pr);
  num_puntos_B = NUM_PUNTOS_B_;

  nk_z = l_interval_z+2*kord-1;    ! # de knots
  ! No incluimos primer y ultimo spline debido a psi(zmin)=psi(zmax)=0
  nb_z = kord+l_interval_z-3;      ! tamaño base splines

  nk_r = l_interval_r+2*kord-1;
  nb_r = kord+l_interval_r-2; ! Incluyo el primer splines

  ! archivo = './resultados/1e-E_vs_B.dat';
  write(archivo, '("./resultados/1e-E_vs_B-v0_",f6.4,"eV.dat")') v0

  open(10, file = archivo1)
  write(10,'(A33,f8.2,A1,f8.2,A4)') "# Intervalo de integracion en z:[", zmin,",", zmax, "] nm"
  write(10,'(A33,f8.2,A1,f8.2,A4)') "# Intervalo de integracion en r:[", zmin,",", zmax, "] nm"
  write(10,'(A37,x,I2,x,A33)') "# Tipo de distribucion de knots en z:", tipo, "(1 es uniforme, 2 es exponencial)"
  write(10,'(A37,x,I2,x,A33)') "# Tipo de distribucion de knots en r:", tipo, "(1 es uniforme, 2 es exponencial)"
  write(10,'(A47,x,f8.6)') "# Cte de decaimiento en distribucion exp beta =", beta
  write(10,'(A30,x,I2)') "# Orden de los bsplines kord =", kord
  write(10,'(A24,x,I3,x,A24,x,I3)') "# Num de intervalos en z", l_interval_z, ", Num de intervalos en r", l_interval_r
  write(10,'(A24,x,I3)') "# Grado de la cuadratura", n_cuad
  write(10,'(A19,x,I3,x,A31,x,I3)') "# Num de knost en z", nk_z, ", tamaño de la base en z nb_z =", nb_z
