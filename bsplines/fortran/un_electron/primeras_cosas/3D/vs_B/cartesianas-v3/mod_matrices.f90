...
subroutine hamiltoniano(nb, v0, me, omega, s, x_mat, p_mat, v_pozo, v_campo, ke, h, ms)
  ...
  real(pr), intent(out) :: h(2*nb**3,2*nb**3), ms(2*nb**3,2*nb**3)
  !!!!
  integer :: i, j, nb3
...
!   ms(1:nb3,1:nb3) = aux2(:,:);
!   ms(nb3+1:2*nb3,nb3+1:2*nb3) = ms(1:nb3,1:nb3);
! !  ms(1:nb3,nb3+1:2*nb3) = ms(1:nb3,1:nb3);
! !  ms(nb3+1:2*nb3,1:nb3) = ms(1:nb3,1:nb3);
!
!
!   h = 0._pr; ms = 0._pr;
!
!   h(1:nb3,1:nb3) = T_cinetica(:,:) - v0*U_pozo(:,:) + me*omega**2*U_campo(:,:);
!   h(nb3+1:2*nb3,nb3+1:2*nb3) = h(1:nb3,1:nb3);
!   h(nb3+1:2*nb3,1:nb3) = -Lz(:,:);
!   h(1:nb3,nb3+1:2*nb3) = Lz(:,:); !-h(nb3+1:2*nb3,1:nb3);

  do j = 1, 2*nb3
    do i = 1, 2*nb3
      if(i<=nb3 .and. j<=nb3) then
        h(i,j) = T_cinetica(i,j) - v0*U_pozo(i,j) + me*omega**2*U_campo(i,j);
        ms(i,j) = aux2(i,j);
      elseif(i<=nb3 .and. nb3<j) then
        h(i,j) = -Lz(i,j-nb3);
        ms(i,j) = 0._pr;
      elseif(nb3<i .and. j<=nb3) then
        h(i,j) = Lz(i-nb3,j);
        ms(i,j) = 0._pr;
      elseif(nb3<i .and. nb3<j) then
        h(i,j) = T_cinetica(i-nb3,j-nb3) - v0*U_pozo(i-nb3,j-nb3) + me*omega**2*U_campo(i-nb3,j-nb3);
        ms(i,j) = aux2(i-nb3,j-nb3);
      end if
    end do
  end do
