...
!###########################################################
!###########################################################
open(9,file='./resultados/autov_vs_lambda.dat')
!###########################################################
!###########################################################
...
real(8), allocatable, dimension(:,:) :: mh, ms, mscopy, mv, hsim, ksim, vsim
...
allocate( ksim(NN,NN), vsim(NN,NN), hsim(NN,NN), ms(NN,NN), mv(NN,NN), mscopy(NN,NN))

!###########################################################
!###########################################################
!###########################################################
open(11,file='./resultados/autov_vs_lambda.dat',position='append')
...
delta = (lambdaf-lambdai)/dble(num_puntos_lambda)


ksim = 0.d0; vsim = 0.d0; mv = 0.d0; ms = 0.d0;
    ...
    do np = 1, dp
      do mp = np, dp
        ...
        if(m.eq.n .and. mp.eq.np)then
          ...
          ksim( ind, indp) = 2.d0*s(n,np)*mh(n,np);! + dble(lambda)*Vef(n,n,np,np);
          vsim( ind, indp) = Vef(n,n,np,np);
          ms( ind, indp) = s(n,np)*s(n,np);
          mv( ind, indp) = Vef(n,n,np,np);

        elseif(m.ne.n .and. mp.eq.np )then

          ksim( ind, indp) = raiz*( 2.d0*s(m,np)*mh(n,np) + 2.d0*s(n,np)*mh(m,np));
          vsim( ind, indp) = raiz*( Vef(n,m,np,np) + Vef(m,n,np,np) );
          ms( ind, indp) = 2.d0*raiz*s(n,np)*s(m,np);
          mv( ind, indp) = 2.d0*raiz*(Vef(n,m,np,np) + Vef(m,n,np,np));

        elseif(m.eq.n .and. mp.ne.np)then

          ksim( ind, indp) = raiz*( 2.d0*s(n,mp)*mh(n,np) + 2.d0*s(n,np)*mh(n,mp));
          vsim( ind, indp) = raiz*( Vef(n,n,np,mp) + Vef(n,n,mp,np) );
          ms( ind, indp) = 2.d0*raiz*s(n,np)*s(n,mp);
          mv( ind, indp) = 2.d0*raiz*(Vef(n,n,np,mp) + Vef(n,n,mp,np));

        else

          ksim( ind, indp) = s(n,np)*mh(m,mp) + s(n,mp)*mh(m,np) &
                          &+ s(m,mp)*mh(n,np) + s(m,np)*mh(n,mp);
          vsim( ind, indp) = 0.5d0*(Vef(n,m,np,mp) + Vef(n,m,mp,np) + Vef(m,n,np,mp) + Vef(m,n,mp,np));
...
do i = 0, num_puntos_lambda

  lambda = lambdai + dble(i)*delta;
 
  hsim(:,:) = 0.d0;

  hsim(:,:) = ksim(:,:) + dble(lambda)*vsim(:,:);
  mscopy(:,:) = ms(:,:);

  call eigen_value( NN, nev, info, hsim, mscopy, e, auvec)
  ...
  !!!!!!!!!!!!!!!!!!!!!!!####################################
  !!!!!!!!!!!!!!!!!!!!!!!####################################
  write(11,6) lambda, (eV*e(m), m = 1, 25)
  write(12,6) lambda, (eV*v(m), m = 1, 25)
  !!!!!!!!!!!!!!!!!!!!!!!####################################
  !!!!!!!!!!!!!!!!!!!!!!!####################################
...
deallocate(e, v, auvec, mh, ms, hsim, ksim, vsim, mv, val_exp)
