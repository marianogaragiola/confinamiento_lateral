...
real(8), allocatable, dimension(:,:) :: s, v01, ke, ke_copy, aux
real(8), allocatable, dimension(:,:,:,:) :: Vef, Vef_copy
...
!###########################################################
!###########################################################
open(9,file='./resultados/auval_vs_B-lambda'//l3//l2//l1//'-reescaleado.dat')
!###########################################################
!###########################################################
...
allocate( norma(nb), s(nb,nb), v01(nb,nb), ke(nb,nb), ke_copy(nb,nb))

allocate( Vef(nb, nb, nb, nb), Vef_copy(nb,nb,nb,nb))

allocate(psi_old(nb*(nb+1)/2), psi_new(nb*(nb+1)/2))

call KNOTS_PESOS(kord, tip, gamma, zmin, zmax, c, l, lum, intg, t, k, x, w, pl)

call matrix_elements_kinetic( );

! call interaccion( );

delta_B = (Bf-Bi)/real(num_puntos_B,kind(0.d0));
...
do ind_B = 0, num_puntos_B
  ...
  call matrix_elements_potential( )
!  ll = a0*ll;
!  ll = ll/a0;
...
deallocate(t, k, norma, s, v01, ke, ke_copy, Vef, Vef_copy)
...
subroutine matrix_elements_potential( )
use carga
use matrices
use integracion
implicit none
      ...
      if( im>0.and.im<nb+1 ) then
        do n = 1,kord
          ...
          if( in>0.and.in<nb+1 )then
            ...
            v01(im,in) = v01(im,in) + w(k(i),j)*sp(m)*sp(n)*(-V0*exp(-(ll*(zz-z0))**2/(2.d0*sigma**2)) )
...
end subroutine matrix_elements_potential

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

subroutine matrix_elements_kinetic( )
use carga
use matrices
use integracion
implicit none
...
end subroutine matrix_elements_kinetic
...
real(8) :: pi, zz1, zz2, w1, w2, const, var
              ...
              if(in>0 .and. in<nb+1)then
                ...
                ! if(zz2.LE.zz1)then
                 ...
                 if( var <= 5.d0) then
                   ...
                   f(im, in) = f(im, in) + Sp(m)*Sp(n)*w2*(1.d0/(zz1-zz2)-1.d0/(zz1-zz2)**3);
                 end if
...
do n = 1, nb
  do np = 1, nb
    ...
    do m = 1, nb
      do mp = 1, nb
        Vef_copy(n,m,np,mp) = Vef_copy(n,m,np,mp)/nor1;
      end do
    end do
  end do
end do
...
do m = 1, nb
  do n = m+1, nb
    ...
    ke_copy(m,n) = ke_copy(m,n)/nor ; ke_copy(n,m) = ke_copy(m,n);
    v01(m,n) = v01(m,n)/nor ; v01(n,m) = v01(m,n);
  end do

  ke_copy(m,m) = ke_copy(m,m)/s(m,m)
...
!###########################################################
!###########################################################
open(11,file='./resultados/auval_vs_B-lambda'//l3//l2//l1//'-reescaleado.dat',position='append')
!open(12,file='./resultados/expectacion_interaccion.dat')
open(13,file='./resultados/fidelidad_vs_B-lambda'//l3//l2//l1//'-reescaleado.dat',position='append')
!###########################################################
!###########################################################
!###########################################################
...
do m = 1, dp
  do n = 1, m
    mh(m,n) = ke_copy(m,n) + v01(m,n) ; mh(n,m) = mh(m,n)
  end do
end do
    ...
    do np = 1, dp
      do mp = np, dp
        ...
        if(m.eq.n .and. mp.eq.np)then
          ...
          hsim( ind, indp) = 2.d0*s(n,np)*mh(n,np) + dble(lambda)*Vef_copy(n,n,np,np);
        ...
        elseif(m.ne.n .and. mp.eq.np )then
          ...
          hsim( ind, indp) = raiz*( 2.d0*s(m,np)*mh(n,np) + 2.d0*s(n,np)*mh(m,np) &
                             & + dble(lambda)*Vef_copy(n,m,np,np) + dble(lambda)*Vef_copy(m,n,np,np) );
          ms( ind, indp) = 2.d0*raiz*s(n,np)*s(m,np);
          mv( ind, indp) = 2.d0*raiz*(Vef_copy(n,m,np,np) + Vef_copy(m,n,np,np));

        elseif(m.eq.n .and. mp.ne.np)then

          hsim( ind, indp) = raiz*( 2.d0*s(n,mp)*mh(n,np) + 2.d0*s(n,np)*mh(n,mp) &
                             & + dble(lambda)*Vef_copy(n,n,np,mp) + dble(lambda)*Vef_copy(n,n,mp,np) );
          ms( ind, indp) = 2.d0*raiz*s(n,np)*s(n,mp);
          mv( ind, indp) = 2.d0*raiz*(Vef_copy(n,n,np,mp) + Vef_copy(n,n,mp,np));

        else

          hsim( ind, indp) = s(n,np)*mh(m,mp) + s(n,mp)*mh(m,np) &
                          &+ s(m,mp)*mh(n,np) + s(m,np)*mh(n,mp) &
                          &+ 0.5d0*dble(lambda)*(Vef_copy(n,m,np,mp)+Vef_copy(n,m,mp,np)+Vef_copy(m,n,np,mp)+Vef_copy(m,n,mp,np));
          ms( ind, indp) = s(n,np)*s(m,mp) + s(n,mp)*s(m,np);
          mv( ind, indp) = 0.5d0*(Vef_copy(n,m,np,mp) + Vef_copy(n,m,mp,np) + Vef_copy(m,n,np,mp) + Vef_copy(m,n,mp,np));
